...
NA stands for Not Available, as n_2021 contains NA values, the mean and median function will always return NA.
...
na.rm is a logical evaluating to TRUE or FALSE, it indicates if NA values should be stripped.
...
When the mean is much larger than the median it means that there are some outliers that makes the data skewed. In this cases the median is a more significant value than the mean. Meaning that the mean is not as useful as the median, since the median represents better the data.
...
visitors_with_nations %>%
  ...
  mutate(PChange = (n_2022-n_2021)/n_2021 *100) %>%
  arrange(admission, PChange)
